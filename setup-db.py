...
import helper_scripts.databricks.db_create_user as db_create_user
...
def setup_learners(
    ...
    ## Commented out since this is only opening a SF connection
    ...
    for count, learner in enumerate(learners):
        ...
        if should_create_users_and_projects:
            if learner["sf_user_created"].upper() == "Y":
                print(
                    f"{learner['sf_username']} already created in Databricks. "
                    "Skipping"
                )
            else:
                db_create_user.main(learner['sf_username'])
    ...
    ## Comment out - only applicable to Snowflake
    # if should_create_users_and_projects:
